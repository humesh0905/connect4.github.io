...
function setWinner(r, c) {
    ...
    if (board[r][c] == playerG) {
        winner.innerText = "Hurrray Green Wins";  
        bodyChangeGreen();
    } 
    else {
        winner.innerText = "Hurrray Orange Wins";
        bodyChangeOrange();
    }
    gameOver = true;
}
...
for (let i = 0; i < greBod.length; i++) {
    greBod[i].style.backgroundColor = 'rgb(194, 252, 156)';
}
}
...
for (let i = 0; i < oraBod.length; i++) {
    oraBod[i].style.backgroundColor = 'rgb(234, 154, 57)';
}
}

function handleKeyDown(event) {
    if (gameOver || currPlayer !== playerG) {
        return;
    }
